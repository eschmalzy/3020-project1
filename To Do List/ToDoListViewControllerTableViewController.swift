...
class ToDoListViewControllerTableViewController: UITableViewController {
    ...
    var toDoList = SampleData.generateToDoData()
    ...
    @IBAction func saveToDoItem(_ segue: UIStoryboardSegue) {
        ...
        guard let itemDetailsViewController = segue.source as? ViewController,
            let item = itemDetailsViewController.toDoItem else {
                return
        }
        if item.item == ""{
            return
